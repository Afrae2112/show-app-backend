...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _navigateToAddShowPage() async {
    final result = await Navigator.push(context, MaterialPageRoute(builder: (context) => const AddShowPage()));
    if (result == true) {
      fetchShows(); // Refresh list after adding a show
    }
  }

  Future<void> _navigateToUpdateShowPage(int showId) async {
    final result = await Navigator.push(context, MaterialPageRoute(builder: (context) => UpdateShowPage(showId: showId)));
    if (result == true) {
      fetchShows(); // Refresh list after updating a show
    }
  }
        ...
        child: ListView(
          children: [
            ...
            ListTile(
              ...
              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (builder)=> const ProfilePage())),
            ),
            ListTile(
              leading: const Icon(Icons.add),
              title: const Text("Add Show"),
              onTap: _navigateToAddShowPage,
            ),
          ],
        ),
      ),
          ...
          child: Card(
            ...
            child: ListTile(
              ...
              trailing: IconButton(
                ...
                onPressed: () {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => UpdateShowPage(showId: show['id']))).then((result) {
                    if (result == true) {
